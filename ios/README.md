...
你已经有 Xcode 和 Apple Developer 账号，目标是把网页版打造成高质量 iOS App。当前目录存放的是一个“可直接导入的起始骨架”，先跑通核心能力，再持续打磨视觉和商业化。

## 本次交付（已产出）

- `ios/TodoNative/`：SwiftUI 应用骨架（模型、视图、VM、服务层）
...
- [x] 7天试用 + 订阅 gating 入口完成（Dashboard / 设置 / 付费弹窗）
- [x] 今日健康度看板（任务完成率、状态分布）上线
- [x] 任务卡片工作流升级（状态快速切换、状态可视化）
- [x] Obsidian 导出面向付费态完善（分享、复制、文件名）
- [x] 单元测试 target 接入（TodoNativeTests，45 用例，iOS 27 全绿）
- [x] iOS 27 (27.0) 模拟器构建 + 启动冒烟验证通过
- [x] 真实 AI 接口接入（OpenAI /v1/responses + gpt-4.1-mini，无 Key 自动本地降级；AI 工作台：智能拆解 / 复盘 / 今日计划）
- [x] 编辑已存任务（TaskEditView）+ 清除已完成
- [x] 中/英双语切换（Localization 字典 + LanguageEnvironment，设置页入口，对齐 web I18N）
...
2. 进入项目后首次可直接构建（你需要本机有可用的 iOS Simulator runtime）：
   - `cd ios`
...
4. 单元测试（32 个用例，覆盖模型 / AI 计划 / Obsidian 导出 / 试用 / ViewModel）：
...
4. 先在本地确认首页与任务流可用，再接入真实 AI 与付费。
5. 再配置 IAP（`com.zhili.todo.premium.monthly` / `com.zhili.todo.premium.yearly`）并启用 7 天试用。
6. 最后补上真实的 AI 后端（或代理服务）后，上 TestFlight。
